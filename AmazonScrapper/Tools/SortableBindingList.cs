...
namespace AmazonScrapper.Tools
{
    public class SortableBindingList<T> : BindingList<T>
    {
        ...
        public SortableBindingList(List<T> list, IComparer comparer = null)
            : base(list)
        {
            _comparer = comparer;
        }
        ...
        protected override void ApplySortCore(PropertyDescriptor prop, ListSortDirection direction)
        {
            ...
            {
                ((List<T>)Items).Sort(new PropertyComparer(prop, direction, _comparer));
        ...
        internal class PropertyComparer : Comparer<T>
        {
            ...
            public PropertyComparer(PropertyDescriptor prop, ListSortDirection direction, IComparer comparer = null)
            {
                ...
                if (CanSortWithIComparable(prop.PropertyType))
                {
                    ...
                    _comparer = comparer ?? (IComparer)property.GetValue(null, null);
                    _useToString = false;
                }
                else
                {
            ...
            public override int Compare(T left, T right)
            {
                ...
                return _direction == ListSortDirection.Ascending ? _comparer.Compare(leftValue, rightValue) : _comparer.Compare(rightValue, leftValue);
            }
