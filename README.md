...
The interface is reminiscent of the ComicVine Scraper, so you should be able to use without too much efforts. The only difference between the former are as follow:
...
-	By default, it will return the book you searched for (versus the ComicVine Scraper that returns series). There is a Group by Series checkbox that you can enable that will group by series, if the information is available in the search results. 
...
-	This will not try to choose the correct book automatically like ComicVine Scraper. You will need to select the correct book every time. The data on Amazon isn’t well organized enough like the former Comixology.
