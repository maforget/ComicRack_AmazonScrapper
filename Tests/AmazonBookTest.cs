...
namespace Tests
{
    ...
    public class AmazonBookTest
    {
        ...
        public void TestGetBook()
        {
            ...
            Assert.IsTrue(book.CommunityRating > 0.0f);
        ...
        public void TestGetBook2()
        {
            ...
            Assert.AreEqual("Rob Guillory, Taylor Wells", book.Penciller);//Artists
            Assert.AreEqual(7.99f, book.BookPrice);
        }
        ...
        public void TestGetBook3()
        {
            ...
            Assert.IsTrue(book.BookPrice > 0.0f);
        }
        ...
        public void TestGetBook9()
        {
            ...
            Assert.AreEqual("Kaiju hit hard. Family hits harder.\r\rTrying to keep your family from imploding is a tall order. Kit Hobbs is about to find out it's an even taller order when that family has been piloting the Titan that protects New Hyperion from kaiju for generations. Between a spiraling brother, a powder keg of a father, and a whole bunch of twenty-story monsters, she's got her work cut out for her.", book.Summary);
		...
		public void TestGetBookFr()
		{
            ...
            Assert.AreEqual("LITTLE URBAN", book.Publisher);
            Assert.IsTrue(book.CommunityRating > 0.0f);
            Assert.AreEqual(@"https://www.amazon.fr/dp/2374081044", book.Web);
		...
		public void TestGetBookFr2()
		{
            ...
            Assert.IsTrue(book.CommunityRating > 0.0f);
            Assert.AreEqual(@"https://www.amazon.fr/dp/2205060694", book.Web);
        }

        [TestMethod]
        public void TestGetBookDe()
        {
            var link = new AmazonLinkIssues("3551799016", tld: TLDs.de);
            var book = link.ScrapeData();

            Assert.AreEqual("Marsupilami 1: Tumult in Palumbien: Abenteuercomics für Kinder ab 8 (1)", book.Title);
            Assert.AreEqual("Marsupilami", book.Series);
            Assert.AreEqual(@"Abenteuer für LeseanfängerDas Marsupilami ist eines der tollsten Tiere des palumbianischen Dschungels! Es ist super intelligent, super stark und es kann super wütend werden, wenn jemand seiner Familie oder dem Dschungel etwas Böses will. Dann macht es aus seinem acht Meter langen Schwanz eine Keule und verteilt Beulen für die Bösewichte!Tumult in PalumbienIn diesem Band kommt der Grosswildjäger Bring M. Backalive nach Palumbien, um das Wundertier Marsupilami zu fangen. Mit einem altersschwachen Boot schippert er auf dem Rio Palumbo durch den Dschungel, um das Nest des Marsupilamis zu finden. Nach vielen gescheiterten Versuchen versucht er es mit einer fiesen List: einem ""trojanischen"" Piranha! Doch das Marsupilami lässt sich nicht für dumm verkaufen und zeigt allen Jägern, dass auch seine Kinder bereits die Technik der ""Schwanzkeule"" beherrschen!Mit dem Marsupilami macht das Lesen lernen richtig Spaß!", book.Summary);

            Assert.AreEqual("André Franquin, Greg", book.Writer);
            Assert.AreEqual("Batem", book.Penciller);

            Assert.AreEqual(30, book.Day);
            Assert.AreEqual(6, book.Month);
            Assert.AreEqual(2015, book.Year);
            Assert.AreEqual(48, book.PageCount);
            Assert.AreEqual("de", book.LanguageISO);
            Assert.AreEqual("Carlsen Comics", book.Publisher);
            Assert.IsTrue(book.CommunityRating > 0.0f);
            Assert.IsTrue(book.BookPrice > 0.0f);
            Assert.AreEqual(@"https://www.amazon.de/dp/3551799016", book.Web);
        }

        [TestMethod]
        public void TestGetBookEs()
        {
            var link = new AmazonLinkIssues("8417760946", tld: TLDs.es);
            var book = link.ScrapeData();

            Assert.AreEqual("Marsupilami 0. ¡Capturad un Marsupilami!", book.Title);
            //Assert.AreEqual("Marsupilami", book.Series);
            Assert.AreEqual(@"Este nuevo volumen fue elaborado para celebrar el 50 aniversario del Marsupilami. Está formado por gags producidos por André Franquin que, durante demasiado tiempo, habían sido inaccesibles para el público. A excepción del ""Nido del Marsupilami"", encontramos aquí todas las historias fundacionales de un universo que permitió a Franquin y a Natem elaborar las nuevas aventuras de este mítico personaje. Por lo tanto, es la base del mito. En estas historietas vemos cómo evoluciona la morfología y el carácter del animal, según los desarrollos que Franquin soñó para él.", book.Summary);

            Assert.AreEqual("André Franquin", book.Writer);
            Assert.AreEqual("Cerise", book.Colorist);
            Assert.AreEqual("David Aliaga Muñoz", book.Translator);

            Assert.AreEqual(26, book.Day);
            Assert.AreEqual(10, book.Month);
            Assert.AreEqual(2020, book.Year);
            Assert.AreEqual(48, book.PageCount);
            Assert.AreEqual("es", book.LanguageISO);
            Assert.AreEqual("BASE", book.Publisher);
            Assert.IsTrue(book.CommunityRating > 0.0f);
            Assert.IsTrue(book.BookPrice > 0.0f);
            Assert.AreEqual(@"https://www.amazon.es/dp/8417760946", book.Web);
        }

        [TestMethod]
        public void TestGetBookBr()
        {
            var link = new AmazonLinkIssues("6525940648", tld: TLDs.br);
            var book = link.ScrapeData();

            Assert.AreEqual("Absolute Superman vol. 01", book.Title);
            //Assert.AreEqual("Marsupilami", book.Series);
            Assert.AreEqual(@"Edição especial de lançamento com preço promocional!Prepare-se para o Universo Absolute! Sem a Fortaleza da Solidão... sem família... sem lar... tudo o que resta é o Homem de Aço Absoluto! Uma ousada reimaginação do Superman pelas mentes criativas dos autores Jason Aaron e Rafa Sandolval!   Leia mais", book.Summary);

            Assert.AreEqual("Jason Aaron", book.Writer);

            Assert.AreEqual(30, book.Day);
            Assert.AreEqual(12, book.Month);
            Assert.AreEqual(2025, book.Year);
            Assert.AreEqual(64, book.PageCount);
            Assert.AreEqual("pt", book.LanguageISO);
            Assert.AreEqual("Panini", book.Publisher);
            Assert.IsTrue(book.CommunityRating > 0.0f);
            Assert.AreEqual(@"https://www.amazon.com.br/dp/6525940648", book.Web);
            Assert.IsTrue(book.BookPrice > 0.0f);
        }
    }
}
